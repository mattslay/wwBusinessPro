...
*-- Note regarding MyBusinessObjects.prg: -------------
...
*-- Base each Parent class off of class wwBusinessPro in wwBusinessPro.prg
*-- Base each Child class off of class wwBusinessProItemList in wwBusinessProItemList.prg
...
*-- parameter in the CreateWWBO() procedure above, or pass .f.
...
loJob.oFileItems.NewItem() && Adds a new child row and new oData object on oFileItems child object.
  ...
  * Option 1: Work on cursor first, then update oData object
  Replace description with 'File description' in (loJob.cFileItemsAlias) && Update a field in the local Child cursor.
  loJob.oFileItems.LoadFromToCurrentRow() && Copies data from loJob.oFileItems.oData to the current row in .cFileItemsAlias

  * Option 2: Work on oData object first, then update cursor row from oData object
